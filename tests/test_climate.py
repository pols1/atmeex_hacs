...
def test_quantize_humidity():
    assert _quantize_humidity(None) == 0
    assert _quantize_humidity(-5) == 0
    assert _quantize_humidity(10) == 0
    assert _quantize_humidity(20) == 33
    assert _quantize_humidity(40) == 33
    assert _quantize_humidity(60) == 66
    assert _quantize_humidity(80) == 66
    assert _quantize_humidity(95) == 100
    assert set(HUM_ALLOWED) == {0, 33, 66, 100}


def _make_entity(state_overrides: dict | None = None):
    cond = {
        "online": True,
        "pwr_on": True,
        "fan_speed": 3,
        ...
        "hum_stg": 1,
        "hum_room": 45,
        "temp_room": 215,   # деци-°C
        "u_temp_room": 225, # деци-°C
    }
    if state_overrides:
        cond.update(state_overrides)
    ...
    entry_id = "entry1"
    ent = AtmeexClimateEntity(coordinator, api, entry_id, "1", "Test Device")

    # hass нужен только для _refresh (refresh_device) — тоже мок
    ent.hass = SimpleNamespace(
        data={
            DOMAIN: {
                entry_id: {"refresh_device": AsyncMock()}
            }
        }
    )
    return ent, cond, api
