...
class AtmeexClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    - Управление температурой (°C) и влажностью (%) — обе цели есть (переключатель на карточке).
    - 7 скоростей вентилятора.
    ...
    def _stage_to_target_humidity(stage: int | None) -> int | None:
        """Ступень увлажнения 0..3 → целевая влажность 0/33/66/100 %."""
        if stage is None:
            return None
    ...
    def _humidity_to_stage(percent: int | float | None) -> int:
        """Проценты влажности → ступень 0..3."""
        if percent is None:
            return 0
        try:
            p = float(percent)
    ...
    async def async_set_temperature(self, **kwargs) -> None:
        ...
        if t is None:
            return
        # Если устройство выключено — включим питание, затем установим цель
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        """Показываем сырое condition как атрибуты — удобно для диагностики в UI."""
        return dict(self._cond) if isinstance(self._cond, dict) else {}from __future__ import annotations
...
class AtmeexClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        """Показываем сырое condition как атрибуты — удобно для диагностики в UI."""
        return dict(self._cond) if isinstance(self._cond, dict) else {}
