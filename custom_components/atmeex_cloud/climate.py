...
# под swing используем режим бризера
BRIZER_SWING_MODES = [
...
class AtmeexClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    @property
    def is_on(self) -> bool:
        return bool(self._cond.get("pwr_on"))from __future__ import annotations
...
# swing_modes мы используем для «режима бризера»
BRIZER_SWING_MODES = [
    "приточная вентиляция",  # 0
    "рециркуляция",          # 1
    "смешанный режим",       # 2
...
class AtmeexClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    _attr_supported_features = (
        ClimateEntityFeature.TARGET_TEMPERATURE
        | ClimateEntityFeature.FAN_MODE
        | ClimateEntityFeature.SWING_MODE  # ← используем под «режим бризера»
    )
    _attr_icon = "mdi:air-purifier"
    _attr_has_entity_name = True
